...
def update(context):
    ...
    if params.tstep % params.plot_result == 0 and solver.rank == 0 and params.plot_result > 0:
        im1.axes.clear()
        im1.axes.contourf(X[1][:, :, 0], X[0][:, :, 0], U[0, :, :, 0], 100)
        im1.autoscale()
        plt.figure(1)
        plt.pause(1e-6)
        im2.axes.clear()
        im2.axes.contourf(X[1][:, :, 0], X[0][:, :, 0], U[1, :, :, 0], 100)
        im2.autoscale()
        plt.figure(2)
        plt.pause(1e-6)
        im3.axes.clear()
        #im3.ax.contourf(X[1, :,:,0], X[0, :,:,0], P[:, :, 0], 100)
        im3.axes.contourf(X[2][:, 0, :], X[0][:, 0, :], U[0, :, 0, :], 100)
        im3.autoscale()
        plt.figure(3)
        plt.pause(1e-6)
...
if __name__ == "__main__":
    ...
    set_Source(**context)
    solver.stats = Stats(context.U, solver.comm, filename='KMMstats')
    context.hdf5file.filename = "KMM665x"
