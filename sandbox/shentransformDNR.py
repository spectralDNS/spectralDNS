...
class ShenRobinBasis(ShenDirichletBasis):
    ...
    def fst(self, fj, fk):
	    ...
	    #fk[:-2] = SFTc.PDMA_3D_complex(a, b, c, fk[:-2])
            fk[:-2] = SFTc.PDMA_II_3D_complex(a, np.hstack([b,0]),np.hstack([c,0,0]),np.hstack([0,b]),np.hstack([0,0,c]), fk[:-2]) 
        elif len(fk.shape) == 1:
	    fk[:-2] = SFTc.PDMA_1D(a, b, c, fk[:-2])
	    #fk[:-2] = SFTc.PDMA_II_1D(a,np.hstack([b,0]),np.hstack([c,0,0]),np.hstack([0,b]),np.hstack([0,0,c]), fk[:-2])
	return fk    
...
if __name__ == "__main__":
    ...
    #t0 = time.time()
    #for i in range(100):
	#d = np.array([3, -2, -1, 3]).astype(float)
	#a = np.array([2, 7, 5,0]).astype(float)
	#b = np.array([1, 1,0,0]).astype(float)
	#c = np.array([0, -3, 2, 2]).astype(float)
	#e = np.array([0, 0, 3, 1]).astype(float)
	#y = np.array([6, 3, 9, 6]).astype(float)
	#y1 = SFTc.PDMA_II_1D(d, a, b, c, e, y)
    #print "Time PDMA_II_1D: ", time.time() - t0
    #print y1

    #t0 = time.time()
    #for i in range(100):
	#mat = diags([e[2:], c[1:], d, a[:-1], b[:-2]], [-2, -1, 0, 1, 2])
	#lu = splu(mat)
	#x = lu.solve(y)
    #print "Time SPLU: ", time.time() - t0
    #print x

    ## Test for tridiagonal
    #print "---------------Tridiagonal-------------"
    #t0 = time.time()
    #for i in range(100):
	#d = np.array([-2.,-2.,-2.,-2.]).astype(float)
	#a = np.array([1.,1.,1.]).astype(float)
	#y = np.array([.04,.04,0.04,0.04]).astype(float)
	#y2 = SFTc.TDMA_1D(a, d, a, y)
    #print "Time TDMA_1D: ", time.time() - t0
    #print y2
	...
	#d = np.array([-2,-2,-2,-2]).astype(float)
	#a = np.array([1.,1.,1.]).astype(float)
	#y = np.array([.04,.04,.04,.04]).astype(float)
	#mat = diags([a,d,a], [-1, 0, 1])
	#lu = splu(mat)
	#x = lu.solve(y)
    #print "Time SPLU: ", time.time() - t0
    #print x

    # Tests for Robin transform
    ...
    # Tests for Neumann transform
    af = np.zeros(N, dtype=np.complex)
    SN = ShenNeumannBasis(quad="GC")
    pointsr, weightsr = SN.points_and_weights(N)
    x = pointsr
    a = x -(1./3.)*(x**3) 
    t0 = time.time()
    af = SN.fst(a, af)
