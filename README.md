...
spectralDNS contains a classical pseudo-spectral Navier-Stokes DNS solver for triply periodic domains. The only unique feature of this solver is that it's written entirely in python using NumPy, MPI for Python (mpi4py) and pyFFTW and, stripping away unnecessary pre- and post-processing steps, the solver is approximately 100 lines long, including the MPI. MPI decomposition is performed using either the "slab" or the "pencil" approach. The code has been found to scale very well in tests on the Shaheen BlueGene/P supercomputer at KAUST Supercomputing Laboratory. Results of both weak and strong scaling tests are shown below. In addition to incompressible Navier-Stokes there are also solvers for MHD and Navier-Stokes or MHD with variable density through a Boussinesq approximation. The solver is described in this paper accepted for publication in Computers Physics Communications: http://arxiv.org/pdf/1602.03638v1.pdf
